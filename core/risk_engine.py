# risk_engine.py
# v1.1.0 - Corregido: Añadido cálculo de log_return para PortfolioEngine
#
# Historial:
# v1.1.0 - Mueve el cálculo de 'log_return' y 'dropna' al motor de riesgo.
# v1.0.0 - Versión inicial

import pandas as pd
import numpy as np  # <-- AÑADIDO PARA CALCULAR LOG_RETURN
import logging
...
class RiskEngine:
    ...
    def compute(self, df_data: pd.DataFrame) -> pd.DataFrame:
        ...
        # <<< SOLUCIÓN AL KEYERROR: 'log_return' >>>
        # Calculamos el retorno logarítmico. Esto es necesario
        # para que el PortfolioEngine pueda calcular retornos ponderados.
        df["log_return"] = np.log(df["close"] / df["close"].shift(1))
        
        # Eliminamos la primera fila (NaN) que no tiene retorno anterior.
        df = df.dropna(subset=["log_return"]) 
        
        if df.empty:
            logging.error("DataFrame se vació después de calcular log_return.")
            return df
        # <<< FIN SOLUCIÓN >>>
        ...
        # --- 4. Calcular Estado de Riesgo y Señal ---
